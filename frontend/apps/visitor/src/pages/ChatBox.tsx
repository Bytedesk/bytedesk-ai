...
// import "@weiyu-ai/chatui/es/styles/index.less";
// // 引入组件
// import Chat, { Bubble, useMessages } from "@weiyu-ai/chatui";
// // 引入样式
// import "@weiyu-ai/chatui/dist/index.css";
//
...
import { EVENT_TOKEN_INVALID } from 'src/utils/constants';
...
export const ChatBox = () => {
  ...
  useEffect(() => {
    if (wid) {
      // 监听token失效
      // function handleTokenInvalid(msg, data) {
      //   console.log("token过期，强制刷新登录", msg, data);
      //   loginToRefreshToken();
      // }
      var handleTokenInvalid = function (msg, data) {
        console.log('token过期，强制刷新登录', msg, data);
        loginToRefreshToken();
      };
      PubSub.subscribe(EVENT_TOKEN_INVALID, handleTokenInvalid);
      // 取消订阅
  ...
  return (
    <Chat
      navbar={{ title: currentWorkGroup.nickname }}
      // FIXME: 第一条消息显示陈旧，未及时更新
      messages={messages}
      isTyping={typing}
      messagesRef={msgRef}
